...
// 	ZEPlayer *zePlayer = g_playerManager->GetPlayer(ms->__m_pChainEntity->m_hController()->GetPlayerSlot());
// 	CBasePlayerPawn *pawn = ms->__m_pChainEntity;
...
void TryPlayerMovePre(CCSPlayer_MovementServices *ms, CMoveData *mv, Vector *pFirstDest, trace_t_s2 *pFirstTrace)
{

	CBasePlayerPawn *playerPawn = ms->__m_pChainEntity;
	ZEPlayer *zePlayer = g_playerManager->GetPlayer(playerPawn->m_hController()->GetPlayerSlot());
	CCSPlayerPawn *pawn = dynamic_cast<CCSPlayerPawn *>(playerPawn); // CCSPlayerPawn *pawn = this->player->GetPawn();

	zePlayer->SetDidTPM(true); 	// this->didTPM = true;

	f32 timeLeft = gpGlobals->frametime;

	Vector start, velocity, end;
	VectorCopy(mv->m_vecVelocity, velocity); // this->player->GetOrigin(&start);
	VectorCopy(mv->m_vecAbsOrigin, start); // this->player->GetVelocity(&velocity);
	...
	CTraceFilterPlayerMovementCS filter;
	addresses::InitPlayerMovementTraceFilter(&filter, pawn, pawn->m_Collision().m_collisionAttribute().m_nInteractsWith(), COLLISION_GROUP_PLAYER_MOVEMENT);

	if (ms->m_bDucked()) // if (this->player->GetMoveServices()->m_bDucked())
	{
		bounds.maxs.z = 54;
	}
	for (bumpCount = 0; bumpCount < MAX_BUMPS; bumpCount++)
	{
		...
		if (zePlayer->GetLastValidPlane().Length() > FLT_EPSILON && (!IsValidMovementTrace(pm, bounds, &filter) || pm.planeNormal.Dot(zePlayer->GetLastValidPlane()) < RAMP_BUG_THRESHOLD))
		{
			zePlayer->SetOverrideTPM(true);
			...
			for (u32 i = 0; i < 3 && !success; i++)
			{
				for (u32 j = 0; j < 3 && !success; j++)
				{
					for (u32 k = 0; k < 3 && !success; k++)
					{
						if (i == 0 && j == 0 && k == 0)
						{
							offsetDirection = zePlayer->GetLastValidPlane();
						}
						else
						{
						...
						{
							addresses::TracePlayerBBox(start + offsetDirection * RAMP_PIERCE_DISTANCE * ratio, end + offsetDirection * RAMP_PIERCE_DISTANCE * ratio, bounds, &filter, pierce);
							if (!IsValidMovementTrace(pierce, bounds, &filter))
							{
								continue;
							}
							// Try until we hit a similar plane.
							validPlane = pierce.fraction < 1.0f && pierce.fraction > 0.1f && pierce.planeNormal.Dot(zePlayer->GetLastValidPlane()) >= RAMP_BUG_THRESHOLD;
							hitNewPlane = pm.planeNormal.Dot(pierce.planeNormal) < NEW_RAMP_THRESHOLD && zePlayer->GetLastValidPlane().Dot(pierce.planeNormal) > NEW_RAMP_THRESHOLD;
						...
						if (goodTrace || hitNewPlane)
						{
							...
							pm.endpos = test.endpos;
							zePlayer->SetLastValidPlane(test.planeNormal);
							success = true;
						}
					}
				}
		...
		{
			zePlayer->SetLastValidPlane(pm.planeNormal);
		}
	...
	zePlayer->SetTPMOrigin(pm.endpos);
	zePlayer->SetTPMVelocity(velocity);
}

void TryPlayerMovePost(CCSPlayer_MovementServices *ms, CMoveData *mv, Vector *pFirstDest, trace_t_s2 *pFirstTrace)
{
	CBasePlayerPawn *playerPawn = ms->__m_pChainEntity;
	ZEPlayer *zePlayer = g_playerManager->GetPlayer(playerPawn->m_hController()->GetPlayerSlot());
	if (zePlayer->GetOverrideTPM())
	{
		VectorCopy(zePlayer->GetTPMOrigin(), mv->m_vecAbsOrigin);
		VectorCopy(zePlayer->GetTPMVelocity(), mv->m_vecVelocity);
	}
}
...
void FASTCALL Detour_CCSPlayer_MovementServices_TryPlayerMove(CCSPlayer_MovementServices *ms, CMoveData *mv, Vector *pFirstDest, trace_t_s2 *pFirstTrace)
{
	...
	{
		TryPlayerMovePre(ms, mv, pFirstDest, pFirstTrace);
		CCSPlayer_MovementServices_TryPlayerMove(ms, mv, pFirstDest, pFirstTrace);
		TryPlayerMovePost(ms, mv, pFirstDest, pFirstTrace);
	}
	else
	{
		CCSPlayer_MovementServices_TryPlayerMove(ms, mv, pFirstDest, pFirstTrace);
...
void FASTCALL Detour_ProcessMovement(CCSPlayer_MovementServices *ms, CMoveData *mv)
{
	if (g_bRampBugFixEnabled)
	{
		CBasePlayerPawn *playerPawn = ms->__m_pChainEntity;
		ZEPlayer *zePlayer = g_playerManager->GetPlayer(playerPawn->m_hController()->GetPlayerSlot());
		zePlayer->SetDidTPM(false);
		zePlayer->SetOverrideTPM(false);
		ProcessMovement(ms, mv);
		if (!zePlayer->GetDidTPM())
		{
			zePlayer->SetLastValidPlane(vec3_origin);
		}
	}
	else
	{
